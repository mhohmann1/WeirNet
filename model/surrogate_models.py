...
class RegDGCNN(nn.Module):
    ...
    def __init__(self, args, output_channels=1):
        ...
        self.linear1 = nn.Linear(args.emb_dims * 2 + 1, 128, bias=False)
    ...
    def forward(self, x, q=None):
        ...
        Args:
            ...
            q (torch.Tensor): Input tensor of shape (batch_size, 1).
        ...
        if q is None:
            q = x.new_zeros((batch_size, 1))
        else:
            q = q.view(batch_size, 1)
        x = torch.cat((x, q), 1)     # (batch_size, emb_dims*2+1)
...
class RegPointNet(nn.Module):
    """
    PointNet-based regression model for 3D point cloud data plus Q.

    Args:
        args (dict): Configuration parameters including 'emb_dims' for embedding dimensions and 'dropout' rate.

    Methods:
        forward(x, q): Forward pass through the network.
    """
    def __init__(self, args):
        ...
        self.linear1 = nn.Linear(args.emb_dims + 1, 512, bias=False)
    ...
    def forward(self, x, q=None):
        ...
        Parameters:
            ...
            q (Tensor): Input tensor of shape (batch_size, 1).
...
class DragGNN_XL(torch.nn.Module):
    ...
    Methods:
        forward(data, q): Forward pass through the network.
    """
    def __init__(self):
    ...
    def forward(self, data: Data, q: torch.Tensor = None) -> torch.Tensor:
        """
        Forward pass through the network.
