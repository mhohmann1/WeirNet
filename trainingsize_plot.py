...
def main():
    parser = argparse.ArgumentParser(description="Plot R2 over training data fraction.")
    parser.add_argument(
        "--metrics-path",
        default="./regression_runs/metrics.csv",
        help="Path to metrics.csv generated by train_regressors.py.",
    )
    parser.add_argument(
        "--output",
        default="./regression_runs/test_r2.png",
        help="Output path for the plot PNG.",
    )
    parser.add_argument(
        "--metric",
        default="test_r2",
        choices=["val_r2", "test_r2"],
        help="Which R2 metric to plot.",
    )
    args = parser.parse_args()

    metrics_df = pd.read_csv(args.metrics_path)
    plot_r2_by_fraction(metrics_df, args.output, args.metric)
